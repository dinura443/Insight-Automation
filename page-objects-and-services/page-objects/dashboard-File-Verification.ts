...
export class VerifyExporter {
  ...
  // Get all top-level export folders (like dashboard_export_*, chart_export_*)
  ...
  // Compare structure of one export folder pair
  private verifyExportPair(folder1Path: string, folder2Path: string): { success: boolean; diff?: any } {
    const result = {
      dashboards: {
        instance1: this.countYamlsInSubdir(path.join(folder1Path, "dashboards")),
        instance2: this.countYamlsInSubdir(path.join(folder2Path, "dashboards")),
      },
      charts: {
        instance1: this.countYamlsInSubdir(path.join(folder1Path, "charts")),
        instance2: this.countYamlsInSubdir(path.join(folder2Path, "charts")),
      },
      databases: {
  ...
  public compare(): { success: boolean; summary?: any } {
    ...
    console.log("✅ All export structures match!");
