  ...
  public compare(): { success: boolean; summary: any } {
    ...
    if (missingInDir2.length || extraInDir2.length) {
      if (missingInDir2.length)
        console.warn(`${colors.yellow}⚠️ Missing in import folder:${colors.reset}`, missingInDir2);
      if (extraInDir2.length)
        console.warn(`${colors.yellow}⚠️ Extra in import folder:${colors.reset}`, extraInDir2);
    } else {
